...
class UploadFileForm(forms.Form):
    ...
    check4 = forms.BooleanField(label="The following names (W1, W3, S1, S3, G1, G3) have been assigned "
                                      "to the SNP primers", required=True)
...
class InstructionsForm(forms.Form):
    check1 = forms.BooleanField(label="Upload a txt file that contains the results.", required=True)
    check2 = forms.BooleanField(label="Analyse only one sample in each qPCR experiment.", required=True)
    check3 = forms.BooleanField(label="Assign to the SNP primers the following names: W1_2554, W3_0222, S1_1033, "
                                      "S3_1292, G1_1884 and G3_0126.", required=True)
